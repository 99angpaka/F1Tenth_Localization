...
struct Node{
    ...
    int prev_k;   // previous index k

    Node(Point _point = Point(0, 0)):point(_point), F(0), G(0), H(0), parent(NULL), prev_k(0)
    {
    }
};
...
public:
    ...
    void PathPlanning(Point _startPoint, vector<double>& _startPointDirection, Point _targetPoint, vector<Point>& path);
...
private:
    ...
    unordered_map<int, Node*> CloseDict; // close dict
